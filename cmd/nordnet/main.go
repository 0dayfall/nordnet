...
func main() {
	...
	username, ok := os.LookupEnv("N_USER")
	if !ok {
		logger.Error("N_USER not set")
		os.Exit(1)
	}

	password, ok := os.LookupEnv("N_PASSWORD")
	if !ok {
		logger.Error("N_PASSWORD not set")
		os.Exit(2)
	}

	pemPath, ok := os.LookupEnv("N_PEM")
	if !ok {
		logger.Error("N_PEM not set")
		os.Exit(3)
	}
